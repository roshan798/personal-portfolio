...
const projectFolders = ['airbnb', 'BST-visualizer', 'i-notebook', 'sudoku', 'trip-easy', 'voicehub']
...
const writeImage = async (inputPath, outputPath, options) => {
  ...
  await sharp(inputPath).resize(options.width).webp({ quality: options.quality }).toFile(outputPath)
  console.log(`Generated ${path.relative(process.cwd(), outputPath)}`)
}

const generateAvatarImages = async () => {
  const avatarOutputs = [
    { width: 944, output: path.resolve('src', 'assets', 'images', 'Designer1.webp') },
    { width: 1888, output: path.resolve('src', 'assets', 'images', 'Designer1@2x.webp') },
  ]

  for (const output of avatarOutputs) {
    await writeImage(avatarSource, output.output, { width: output.width, quality: 80 })
  }
}
...
const generateProjectImages = async () => {
  for (const folder of projectFolders) {
    ...
    await writeImage(inputPath, `${outputBase}.webp`, { width: 400, quality: 80 })
    await writeImage(inputPath, `${outputBase}@2x.webp`, { width: 800, quality: 80 })
  }
}
